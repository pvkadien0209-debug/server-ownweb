...
module.exports = (jsonParser) => {
  ...
  router.post("/split-audio", jsonParser, async (req, res) => {
    ...
    try {
      ...
      // Cắt các segments với tên từ mp3Code
      const promises = segments.map((seg, i) => {
        // ✨ SỬ DỤNG CODE TỪ mp3Code HOẶC FALLBACK NẾU KHÔNG CÓ
        const fileName = mp3Code[i]?.code
          ? `${mp3Code[i].code}.mp3`
      ...
      return res.json({
        ...
        isMatched: files.length === mp3Code.length,
