...
const speedRate = 1.7;
const pitchShift = 1.7; // Giảm pitch để có giọng nam (0.8-0.9 là phù hợp)
...
// Hàm tăng tốc độ audio và thay đổi pitch để có giọng nam bằng ffmpeg
async function speedUpAudioWithPitch(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      console.log(
        `    → Processing audio: Speed ${speedFactor}x, Pitch ${pitch}x (male voice)`
      );

      // Sử dụng ffmpeg để tăng tốc độ VÀ thay đổi pitch
      // rubberband filter giữ chất lượng tốt hơn khi thay đổi pitch
      // atempo cho tốc độ, asetrate+atempo cho pitch
      const pitchRatio = pitch;
      const sampleRateMultiplier = 1 / pitchRatio;

      ffmpeg(tempInputPath)
        // Phương pháp 1: Dùng asetrate và atempo để thay đổi pitch mà không thay đổi tốc độ
        .audioFilters([
        ...
        .on("end", () => {
          ...
          console.log(
            `    → Audio processing completed (male voice with ${speedFactor}x speed)`
          );
          resolve();
        })
        .on("error", (err) => {
...
// Hàm xử lý một item text thành MP3 với tốc độ tăng cường và giọng nam
async function processTextToMp3(item, index, total) {
  ...
  try {
    ...
    // Tăng tốc độ audio và thay đổi pitch để có giọng nam
    ...
    console.log(
      `  ✅ Created: ${item.code}.mp3 (${(finalStats.size / 1024).toFixed(
        2
      )} KB) with ${speedRate}x speed and male voice`
    );
...
module.exports = (jsonParser) => {
  ...
  router.post("/ttslistTV", jsonParser, async (req, res) => {
    console.log(
      "🎵 TTS Tiếng Việt processing started (Male voice with ${speedRate}x speed)"
    );
    ...
    try {
      ...
      console.log(
        `🎙️  Voice: Male (pitch ${pitchShift}x, speed ${speedRate}x)`
      );
