...
async function createAudioBuffer(text) {
  try {
    ...
    const url = googleTTS.getAudioUrl(text, {
      lang: "vi", // Đổi từ "vn" thành "vi" để đúng chuẩn
      slow: false,
      host: "https://translate.google.com", // Đảm bảo sử dụng đúng host
    });
...
// Hàm tăng tốc độ audio bằng ffmpeg
async function speedUpAudio(inputBuffer, outputPath, speedFactor = speedRate) {
    ...
    try {
      ...
      ffmpeg(tempInputPath)
        ...
        .on("end", () => {
          ...
          console.log(`    → Speed enhancement completed`);
          resolve();
        })
        .on("error", (err) => {
...
// Hàm xử lý một item text thành MP3 với tốc độ tăng cường
async function processTextToMp3(item, index, total) {
  ...
  try {
    ...
    // Tăng tốc độ audio lên speedRatex
    await speedUpAudio(originalBuffer, filePath, speedRate);
...
module.exports = (jsonParser) => {
  ...
  router.post("/ttslistTV", jsonParser, async (req, res) => {
    console.log(
      "🎵 TTS Tiếng Việt processing started (with speedRatex speed enhancement)"
    );
    ...
    try {
      ...
      console.log(`🚀 Speed enhancement: speedRatex applied to all files`);
      ...
      res.status(200).json({
        success: true,
        message:
          "TTS Tiếng Việt processing completed with speedRatex speed enhancement",
    ...
    try {
      ...
      if (result.success) {
        res.status(200).json({
          success: true,
          message:
            "Single TTS file created successfully with speedRatex speed enhancement",
          language: "Vietnamese (vi)",
          speedFactor: speedRate,
          result: result,
          file: `${code}.mp3`,
  ...
  router.get("/ttslistTV/files", (req, res) => {
    try {
      const files = fs
        ...
        .sort((a, b) => a.code.localeCompare(b.code)); // Sắp xếp theo code
  ...
  router.get("/ttslistTV/play/:code", (req, res) => {
    ...
    try {
      ...
      res.set({
        ...
        "X-Speed-Factor": "speedRate", // Custom header để biết file đã được tăng tốc
      });
